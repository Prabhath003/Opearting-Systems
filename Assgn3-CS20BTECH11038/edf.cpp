...
class Priority
{
    public:
        ...
        {
            if(p1.next_deadline == p2.next_deadline)
            {
                return p1.remaining_time < p2.remaining_time;
            }
            
            return p1.next_deadline < p2.next_deadline;
        }
};
...
int main()
{
    ...
    std::string log = "EDF-Log.txt";
    std::string stat = "EDF-Stats.txt";
    ...
    for(std::set<int>::iterator it = deadline_check_points.begin(); it != --deadline_check_points.end(); it++)
    {
        ...
        if(free == true)
        {
            ...
            for(int itr = 0; itr < num_of_processes; itr++)
            {
                ...
                if(*it % processes[itr].period == 0 && processes[itr].repetitions > 0)
                {
                    ...
                    {
                        if(processes[itr].ID == waitingQueue[i].ID)
                        {
                            ...
                            for(unsigned j = 0; j < record.size(); j++)
                            {
                                ...
                                {
                                    record[j].waitingTime.push_back(currentProcess.waitingTime);
                                    break; 
                                }
                            }
    ...
    for(unsigned i = 0; i < record.size(); i++)
    {
        statFile << "Process " << record[i].ID << ": Average waiting time: " << std::accumulate(record[i].waitingTime.begin(), record[i].waitingTime.end(), 0) / (float)record[i].waitingTime.size() << std::endl;
    }
